...
async function fetchOffsetLookupJson(error = false): Promise<IOffsetsLookup> {
...
async function fetchOffsetsJson(is_64bit: boolean, filename: string, error = false): Promise<IOffsets> {
	const url = error ? BASE_URL_error : BASE_URL;
