...
export async function GenerateHat(imagePath: URL, colors: string[][], colorId: number, path: string) {
	try {
		...
		try {
			// Instead of a synchronous existsSync which blocks the event loop,
			// we attempt to stat the file asynchronously.
			// This performs a single filesystem operation when the file exists.
			const stats = await fs.promises.stat(temp);
			shouldGenerate = Date.now() - stats.mtimeMs > 300000;
		} catch (statError: any) {
			// If the file does not exist, stat throws an ENOENT error.
			// In that case, we need to generate the image.
